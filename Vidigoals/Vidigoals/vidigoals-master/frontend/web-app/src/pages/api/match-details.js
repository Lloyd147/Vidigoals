...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Group assists by player with count: "B.Fernandes (2)" format
    function groupAssistsByCount(entries) {
      const map = {};
      for (const e of entries) {
        if (!map[e.player]) map[e.player] = 0;
        map[e.player] += e.value || 1;
      }
      return Object.entries(map).map(([player, count]) => ({
        player,
        minute: `${count}`,
      }));
    }
      ...
      if (fplFixtures) {
        ...
        if (bootstrap) {
          ...
          if (fplHome && fplAway) {
            ...
            if (fplFixture && fplFixture.stats) {
              ...
              // Helper to extract stat entries
              function extractStat(identifier, limit) {
                const stat = fplFixture.stats.find(s => s.identifier === identifier);
                if (!stat) return { home: [], away: [] };
                const homeEntries = swapped ? stat.a : stat.h;
                const awayEntries = swapped ? stat.h : stat.a;
                const home = [];
                const away = [];
                for (const entry of (homeEntries || [])) {
                  const player = playerMap[entry.element];
                  if (player) home.push({ player: player.web_name, value: entry.value });
                }
                for (const entry of (awayEntries || [])) {
                  const player = playerMap[entry.element];
                  if (player) away.push({ player: player.web_name, value: entry.value });
                }
                home.sort((a, b) => b.value - a.value);
                away.sort((a, b) => b.value - a.value);
                if (limit) {
                  return { home: home.slice(0, limit), away: away.slice(0, limit) };
                }
                return { home, away };
              }

              // Bonus points (final 3/2/1 allocation)
              const bonusData = extractStat('bonus');
              bonus = bonusData;

              // BPS (running bonus point system scores) — top 5 per side
              const bpsData = extractStat('bps', 5);
              bps = bpsData;

              // Defensive Contribution — direct from FPL, top 5 per side
              const dcData = extractStat('defensive_contribution', 5);
              defensiveContribution = dcData;

              // FPL Assists — authoritative, override API-Football assists
              const fplAssistData = extractStat('assists');
              if (fplAssistData.home.length > 0 || fplAssistData.away.length > 0) {
